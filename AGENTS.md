...
역할 하나가 에이전트 호출 하나를 뜻하지 않는다. 소규모 추출·통합·소재매핑·적합도·일반 수정은 메인이 수행하고, 공식 검증과 JD 구조화는 개인 프로필을 읽지 않는 한 작업으로 묶을 수 있다. 큰 독립 묶음만 기본 동시 2개로 위임한다. 사용자가 요청한 사실 검수와 제출 전 final을 실행할 때만 작성자와 검수자를 분리한다. 명백한 자격 불충족은 공식 공통 요건과 한 줄 사유에서 멈추고 긴 직무 보고서나 개인 경험 재비교를 만들지 않는다.

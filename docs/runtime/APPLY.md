...
기존 문장 수정은 해당 본문·배정 근거와 checkpoint에서 바로 시작한다. catalog·매핑·prepare는 새 문항이나 입력 변경이 있을 때만 필요하다. packet이 없는 기존 파일은 독립 형식 검사를 사용한다. 현재 메인이 간단한 매핑·작성·수정을 끝내고, 명백히 큰 독립 작업만 예외적으로 위임한다.
...
claim 입력 해시는 배정한 claim 내용과 공유 문맥·역할·금지 표현을 기준으로 한다. 같은 경험 파일의 미배정 claim 변경이나 행 번호 이동만으로는 재사용을 취소하지 않는다. 원문 전체 해시는 출처 추적용으로 packet에 유지하며, 선택 claim·공유 문맥·JD·문항·자격이 바뀌면 기존 PASS를 재사용하지 않는다. 이전 전체 파일 해시 방식의 packet은 처음 prepare할 때 보수적으로 무효화될 수 있으며, 옛 PASS의 해시를 새 값으로 자동 바꿔 이어받지 않는다.

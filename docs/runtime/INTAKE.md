...
intake는 최초 정리 뒤 새 경험·사용자 정정만 갱신한다. 새 파일 추출 없이 사용자 답변으로 소수 claim을 정정하는 경우에는 해당 사실·요약·확인 근거만 수정하고 plan/run 절차를 선행하지 않는다. 아래 계약은 파일 추출·재개에 적용한다. 해당 `.work/intake/<run-id>/run.json`이 있으면 `status`로 재개 위치를 확인한다. 완료 작업을 다시 시키지 않고, 실행 중 worker는 플랫폼의 실제 상태를 확인한다.
...
새로 만드는 경험은 [경험 정본 계약](EXPERIENCE.md)의 experience-v2 형식으로 쓴다. 경험 유형에 따라 필요한 절이 다르며 자격·수업·활동에는 문제–행동 서사를 강요하지 않는다. 기존 경험은 일괄 변환하지 않고, 실제로 다시 사용해 근거를 보강할 때만 해당 파일에 표시를 붙이고 확인된 필드부터 채운다. 형식을 올린다는 이유로 claim-id나 `검증됨 · 확인 필요 · 사용 금지` 상태를 바꾸지 않는다. `validate-profile.mjs`는 구조만 검사하며 의미 검토를 대신하지 않는다.

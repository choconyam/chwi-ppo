...
내용을 확정한 뒤 `final`을 요청하면 별도의 읽기 전용 검수 역할이 현재 전체 본문의 JD 충족, 사실·수치·역할, 문항 간 일관성을 한 번 독립적으로 확인합니다. 이때 중간 사실 검수를 다시 선행하지 않습니다. 지적 수정이 있으면 해당 부분과 변경 영향만 한 번 재검수하고, 통과한 문서만 민감정보 검사 뒤 `최종/`에 저장합니다.

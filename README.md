...
내 경험을 정리하고 채용 포털의 공고 목록에서 나에게 맞는 기업·직무를 추린 뒤, 공식 JD 확인과 지원서 초안·제출 전 최종 검수까지 이어 가는 로컬 에이전트 워크벤치입니다. 신입과 경력 지원자 모두 사용할 수 있습니다. Codex와 Claude Code에서 같은 워크플로를 실행합니다.
...
자세한 내용은 [1.1.1 변경 내역](docs/releases/v1.1.1.md)을 확인하세요.
...
- **지원서 재작업 축소**: 형식 검사된 검수 전 초안을 먼저 보여 주고 변경된 부분만 고칩니다. 내용이 달라지면 이전 최종 검수 결과를 그대로 사용하지 않습니다.
...
apply 회사 직무
    ↓
근거 매핑·검수 전 초안·요청 시 사실 검수·최종 검수
    ↓
track (선택)
    ↓
작성·제출 상태와 마감 관리
...
공식 근거를 확인하지 못한 정보는 확정하지 않고 `공식 확인 필요`로 남깁니다. 일부 페이지만 읽었다면 확인 범위와 남은 후보를 함께 알려주고 이어서 검색합니다. 공식 공통 자격이 명백히 맞지 않는 직무는 요건과 짧은 사유만 알려주고 긴 분석을 만들지 않습니다.
...
`/apply`는 선택한 직무의 JD와 `intake`에서 검증된 경험을 연결합니다. 문항마다 사용할 근거를 먼저 배정하고 글자수·형식을 검사한 `검수 전 초안`을 바로 보여 줍니다. 별도 사실 검수는 요청할 때만 실행하며, 일반 수정은 요청한 부분만 바꿉니다. 원자료에 없는 경험·성과를 추가하거나 진행 중인 일을 완료로 바꾸지 않습니다. 로그인 뒤에만 보이는 문항은 해당 문항과 글자수를 붙여주면 됩니다. 정확히 1000자처럼 분량을 맞춰 달라고 하지 않았다면 최대 글자수 이내에서 자연스럽게 작성합니다.
...
내용을 확정한 뒤 실제 제출 전 전체 검수·제출본 확정 의미로 `final`을 요청하면 별도의 읽기 전용 역할이 현재 전체 본문의 문항/자격 충족, 사실·수치·기간·역할·상태, 문항 간 모순처럼 제출에 영향 있는 결함만 한 번 확인합니다. “최종 문장 좀 바꿔” 같은 일반 편집은 final 호출이 아닙니다. 지적 수정 뒤에는 해당 지적과 변경 영향만 한 번 재확인하며, 문체 취향·학술 재감사·면접 대비는 포함하지 않습니다. 면접 준비는 서류 합격 뒤 별도로 요청할 수 있습니다.
...
최종 문장이 번역투이거나 AI 문체처럼 읽힌다면 final을 요청하기 전에 [humanizer-ko](https://github.com/choconyam/humanizer-ko)를 명시적으로 요청해 쓸 수 있습니다. 사실과 수치는 그대로 두고 한국어 문체만 다듬는 별도 도구이며, 이 파이프라인의 필수 구성은 아닙니다.

...
사용한 경험 정본과 실제 문항만 읽고 불일치에 한해서 원천 근거를 추가 확인한다. 문항별로 PASS/REVISE/BLOCK을 기록한다. 각 검수 보고서에는 `- 검수 단계: fact`, `- 판정:`, `- 입력 해시:`, `- 본문 해시:`를 넣는다. 입력 해시는 packet의 해당 문항 값, 본문 해시는 `node scripts/apply-packet.mjs hash --file <검수파일>` 값이다. 해시 확인은 의미 검수를 대신하지 않는다.

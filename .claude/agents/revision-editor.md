...
- 입력: 현재 초안 또는 수정본, 사용자가 반영을 요청한 `05_사실검수_<문항ID>.md` 또는 `07_최종검수_<문항ID>.md`, 원천 경험 Markdown
...
6. 한 번의 수정 범위에서 지적 사항을 함께 처리하고 자동 재수정 루프를 시작하지 않는다.

검수에 근거 없는 요청이 있거나 원자료가 부족하면 수정하지 말고 `intake 갱신 필요`로 보고한다.

## 변경 범위

지적된 문항만 읽고 수정한다. 파일 복사만 필요한 PASS 문항이나 사용자의 일반 문장 수정에는 이 역할을 호출하지 않는다. 최종 검수 지적을 반영한 본문은 새 해시로 기존 지적·변경 영향·문항 간 일관성을 재검수받고 이전 PASS를 재사용하지 않는다. 재검수 뒤에도 문제가 남으면 보고하고 자동 반복하지 않는다.
